...
func newNodesV1(rootSDK *HathoraCloud, sdkConfig config.SDKConfiguration, hooks *hooks.Hooks) *NodesV1 {
	return &NodesV1{
		rootSDK:          rootSDK,
...
func (s *NodesV1) GetNode(ctx context.Context, nodeID string, opts ...operations.Option) (*components.NodeV1, error) {
	...
	opURL, err := utils.GenerateURL(ctx, baseURL, "/nodes/v1/{nodeId}", request, nil)
	if err != nil {
...
func (s *NodesV1) ListProvisionedNodes(ctx context.Context, fleetID string, orgID *string, region *components.Region, opts ...operations.Option) ([]components.NodeV1, error) {
	request := operations.ListProvisionedNodesRequest{
		FleetID: fleetID,
		OrgID:   orgID,
		Region:  region,
	}

	globals := operations.ListProvisionedNodesGlobals{
	...
	opURL, err := utils.GenerateURL(ctx, baseURL, "/nodes/v1/fleet/{fleetId}/listProvisioned", request, globals)
	if err != nil {
		return nil, fmt.Errorf("error generating URL: %w", err)
	}
	...
	hookCtx := hooks.HookContext{
		...
		BaseURL:          baseURL,
		Context:          ctx,
		OperationID:      "ListProvisionedNodes",
		SecuritySource:   s.sdkConfiguration.Security,
	}
