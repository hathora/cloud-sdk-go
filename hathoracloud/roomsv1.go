...
func newRoomsV1(rootSDK *HathoraCloud, sdkConfig config.SDKConfiguration, hooks *hooks.Hooks) *RoomsV1 {
	return &RoomsV1{
		rootSDK:          rootSDK,
...
func (s *RoomsV1) GetConnectionInfoDeprecated(ctx context.Context, roomID string, appID *string, opts ...operations.Option) (*components.ConnectionInfo, error) {
	...
	if retryConfig != nil {
		...
		if err != nil || httpRes == nil {
			...
			_, err = s.hooks.AfterError(hooks.AfterErrorContext{HookContext: hookCtx}, nil, err)
			return nil, err
		} else if utils.MatchStatusCodes([]string{"400", "402", "404", "408", "429", "4XX", "500", "5XX"}, httpRes.StatusCode) {
			_httpRes, err := s.hooks.AfterError(hooks.AfterErrorContext{HookContext: hookCtx}, httpRes, nil)
			if err != nil {
				return nil, err
			} else if _httpRes != nil {
